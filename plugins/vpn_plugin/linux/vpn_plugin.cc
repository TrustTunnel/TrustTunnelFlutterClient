...
#define VPN_PLUGIN(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), vpn_plugin_get_type(), \
                              VpnPlugin))
...
static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
                           gpointer user_data) {
  VpnPlugin* plugin = VPN_PLUGIN(user_data);
  vpn_plugin_handle_method_call(plugin, method_call);
}

void vpn_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
  VpnPlugin* plugin = VPN_PLUGIN(
      g_object_new(vpn_plugin_get_type(), nullptr));
